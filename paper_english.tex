...
To provide detailed error analysis while maintaining clarity, we focus on LLaVA-1.5 on COCO-POPE as a representative example (similar patterns are observed on A-OKVQA). Figure~\ref{fig:confusion_pope} presents confusion matrices comparing baseline and VCD+AGLA. The combined method significantly reduces both false positives (FP: 140$\rightarrow$88, -37.1\%) and false negatives (FN: 397$\rightarrow$333, -16.1\%), resulting in a 21.6\% reduction in total errors.
...
\begin{table}[htbp]
\centering
\caption{Ablation study on component contributions (LLaVA-1.5). COCO-POPE and Hallucinogen-ID are shown as representative examples; similar patterns are observed across other subsets.}
